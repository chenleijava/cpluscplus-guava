...
std::shared_ptr<spdlog::logger> astrolog_logger = Logger::createLogger("astrolog_logger");
void d(std::string &&flg,std::string &&msg){
astrolog_logger->info("{}:{}",flg,msg);
}
...
#ifdef GRAPH
  //绘制图片
  /* If any of the X window switches in effect, */
  /* then go make a graphics chart...           */
  if (us.fGraphics)         {
   // d("GRAPH ","开始绘制图片FActionX()");
    FActionX();
    //d("GRAPH ","开始绘制图片FActionX()---- DONE");
  } else
#endif
    {
        if (gs.fInverse) {
...
flag FProcessSwitches(int argc, char **argv)
{
  ...
  real rT;
  char ch1, ch2, *pch;
  CI ci; //时间 年月日 小时  夏时令 经度 维度 chartName， 位置
...
/* Initialize program variables and tables that aren't done so at compile time.
 * Called once when the program starts from main() or WinMain(). */
void InitProgram()
{
  int i;
...
//  char sz[cchSzMax];
//#endif
//  d("InitProgram()","Read in info from the astrolog.as file.>>>>>>>>>>>>>>>>");
//  /* Read in info from the astrolog.as file. */
//  InitProgram();
//  FProcessSwitchFile(DEFAULT_INFOFILE, NULL);
//  d("InitProgram()","Read in info from the astrolog.as file done>>>>>>>>>>>>>>>>");
//#ifdef BETA
...
//  }
//  is.szProgName = argv[0];  // 文件路径
//  int flg=FProcessSwitches(argc, argv);
//  if (flg) {
...
//
//    d("argv",*argv);
//    d("action()","begin to action>>>>>>>>>>>>>>>>");
//    Action();
//    d("action()","end to action>>>>>>>>>>>>>>>>");
//  }
//
//
//
