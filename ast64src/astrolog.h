...
typedef struct _InternalSettings {
  ...
  flag fDst;          /* Has Daylight Saving Time been autodetected?  夏时令是否自动检查      */
  flag fProgress;     /* Are we doing a chart involving progression?   开始处理 chart绘制     */
  flag fReturn;       /* Are we doing a transit chart for returns?         */
  flag fMult;         /* Have we already printed at least one text chart? 是否已经打印文本  */
  flag fSeconds;      /* Do we print locations to nearest second?          */
  flag fSzPersist;    /* Are parameter strings persistent when processing? 处理时参数字符串是否持久 */
  flag fSzInteract;   /* Are we in middle of chart so some setting fixed?  */
  flag fNoEphFile;    /* Have we already had a ephem file not found error? ephem文件错误*/
  ...
  real Asc; /* Ascendant at chart time. 上升时间 */
  real RA;  /* Right ascension at time.  提升 */
  real OB;  /* Obliquity of ecliptic.   黄道的倾斜 */
  char data[5000];
} IS;

/***
 * char_info
 * 年月日  小时 日光偏移 时区 经度 维度 chartName， 位置
 */
